...
var color = d3.scaleLinear().domain([0, 1/4*5000000, 2/4*5000000, 3/4*5000000, 5000000]).range(["#0576b5", "#6280a4", "#445b75", "#3155d4"]);
...
d3.json("assets/data/us.json", function(data) {
    ...
    function display(d) {
        ...
        document.forms[0].addEventListener("change", function() {
            ...
            .sum(function (d) {
                if (treeSumSortType == "number") {
                    color = d3.scaleLinear().domain([0, 1/4*5000000, 2/4*5000000, 3/4*5000000, 5000000]).range(["#0576b5", "#6280a4", "#445b75", "#3155d4"]);
                    return d["Total College"];
                } else if (treeSumSortType == "percent") {
                    color = d3.scaleLinear().domain([0, 1/4*50, 2/4*50, 3/4*50, 50]).range(["#ab0808", "#8f0707", "#720505", "#560404"]);
                    return d["Percent College"];
                } else if (treeSumSortType == "male") {
                    color = d3.scaleLinear().domain([0, 1/4*50, 2/4*50, 3/4*50, 50]).range(["#bada55", "#00784a", "#82cfa4", "#49d810"]);
                    return d["Percent College - Male"];
                } else {
                    color = d3.scaleLinear().domain([0, 1/4*50, 2/4*50, 3/4*50, 50]).range(["#ffdd73", "#fff4b9", "#ffd700", "#f0bd44"]);
